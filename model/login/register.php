<?php
	...
	if(isset($_POST['registerUsername'])){
		...
		$registerRole = isset($_POST['registerRole']) ? htmlentities($_POST['registerRole']) : '';
		$registerPhone = isset($_POST['registerPhone']) ? htmlentities($_POST['registerPhone']) : '';
		$registerdestination = isset($_POST['registerdestination']) ? htmlentities($_POST['registerdestination']) : '';
		...
		if(!empty($registerFullName) && !empty($registerUsername) && !empty($registerPassword1) && !empty($registerPassword2)){
			...
			if($usernameCheckingStatement->rowCount() > 0){
				...
				if($registerPassword1 !== $registerPassword2){
					...
					$insertUserSql = 'INSERT INTO user(fullName, username, role, phone, destination, password) VALUES(:fullName, :username, :role, :phone, :destination, :password)';
