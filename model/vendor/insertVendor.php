<?php
	...
	if(isset($_POST['vendorDetailsStatus'])){
		...
		/*if(isset($fullName) && isset($mobile) && isset($address)) {
			...
			$sql = 'INSERT INTO vendor(city, district) VALUES(:city, :district)';
			$stmt = $conn->prepare($sql);
			$stmt->execute(['city' => $city, 'district' => $district]);
			echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>destination added to database</div>';
